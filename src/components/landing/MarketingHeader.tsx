        ...
        <header className="relative mx-auto max-w-7xl px-6 py-5">
          <div className="flex items-center justify-between gap-6">
            ...
            <nav className="hidden items-center gap-4 md:flex">
              <Link href="/services" className={`whitespace-nowrap text-sm font-medium transition-colors ${isActive("/services") ? "text-brand-primary" : "text-ink-soft hover:text-ink"}`}>Nos services</Link>
              <div className="relative" onMouseEnter={() => setPayrollOpen(true)} onMouseLeave={() => setPayrollOpen(false)}>
                <button type="button" aria-expanded={payrollOpen} aria-haspopup="menu" onClick={() => setPayrollOpen((value) => !value)} className={`inline-flex whitespace-nowrap items-center gap-1 text-sm font-medium transition-colors ${payrollActive ? "text-brand-primary" : "text-ink-soft hover:text-ink"}`}>
                  Gestion de la paie
                    ...
                    <div className="max-h-[75vh] overflow-y-auto rounded-2xl border border-surface-border bg-white p-2 shadow-xl">
                      <div className="grid grid-cols-2 gap-1">
                        {PAYROLL_LINKS.map((link) => (
                          <Link key={link.href} href={link.href} onClick={() => setPayrollOpen(false)} className={`block rounded-xl px-3 py-2.5 text-sm transition-colors ${isActive(link.href) ? "bg-brand-primary/10 font-medium text-brand-primary" : "text-ink-soft hover:bg-surface-subtle hover:text-ink"}`}>
                            {link.label}
                          </Link>
                        ))}
                      </div>
              ...
              {NAV_LINKS.slice(1).map((link) => (
                <Link key={link.href} href={link.href} className={`whitespace-nowrap text-sm font-medium transition-colors ${isActive(link.href) ? "text-brand-primary" : "text-ink-soft hover:text-ink"}`}>{link.label}</Link>
              ))}
              <span aria-hidden className="h-4 w-px bg-surface-border" />
              <Link href="/sign-in" className={`whitespace-nowrap text-sm font-medium transition-colors ${isActive("/sign-in") ? "text-brand-primary" : "text-ink-soft hover:text-ink"}`}>Se connecter</Link>
              <Link href="/sign-up" className="shrink-0"><Button>Essayer gratuitement</Button></Link>
            </nav>

            <button type="button" onClick={() => setOpen((o) => !o)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-surface-border text-ink-soft md:hidden" aria-label={open ? "Fermer le menu" : "Ouvrir le menu"} aria-expanded={open}>
          ...
          {open && (
            <nav className="absolute inset-x-6 top-full z-50 mt-3 flex max-h-[80vh] flex-col gap-1 overflow-y-auto rounded-2xl border border-surface-border bg-white p-3 shadow-xl md:hidden">
              ...
              <div className="rounded-xl bg-surface-subtle/60 px-2 py-2">
                <Link href="/gestion-paie" onClick={() => setOpen(false)} className={`block rounded-lg px-2 py-2 text-sm font-medium ${payrollActive ? "text-brand-primary" : "text-ink"}`}>Gestion de la paie</Link>
