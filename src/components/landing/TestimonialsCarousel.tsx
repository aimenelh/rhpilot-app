...
const TESTIMONIALS = [
  { quote: "La tâche en RH est la gestion de la paie que j’ai le plus peur d’oublier.", source: "Professionnelle RH", avatar: ANON_AVATARS.alicia, featured: true },
  { quote: "Les échéances liées à la gestion de la paie se déroulent chaque fin de mois et mettent les RH sous tension.", source: "RH en alternance", avatar: ANON_AVATARS.zelia },
  { quote: "Il manquait parfois juste un rappel : une visite médicale, une pièce d’identité arrivée à expiration, ou un bon suivi après le recrutement.", source: "Professionnelle RH", avatar: ANON_AVATARS.justine },
  { quote: "Le suivi des échéances RH (contrats, visites médicales, entretiens obligatoires) demande beaucoup de rigueur. Un oubli peut vite avoir des conséquences.", source: "Professionnelle RH", avatar: ANON_AVATARS.naomy },
  { quote: "Certaines entreprises en auraient bien besoin !", source: "Professionnelle RH", avatar: ANON_AVATARS.emma },
];
...
function Avatar({ src, clear = false }: { src: string; clear?: boolean }) {
  return (
    <span className="relative block h-10 w-10 shrink-0 overflow-hidden rounded-full border border-surface-border bg-surface-subtle">
...
function TestimonialCard({ quote, source, avatar, featured = false }: { quote: string; source: string; avatar: string; featured?: boolean }) {
  return (
    <article className={`self-start rounded-2xl border p-6 ${featured ? "border-brand-primary/20 bg-brand-primary/[0.035] shadow-card" : "border-surface-border bg-white"}`}>
      <div className="flex items-center gap-3">
        <Avatar src={avatar} />
        <div className="min-w-0">
          <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-brand-primary">Retour terrain</p>
          <p className="mt-0.5 text-xs font-medium text-ink-soft">{source}</p>
        </div>
      </div>
      <div className={`mt-6 ${featured ? "text-xl font-medium leading-8" : "text-[15px] leading-7"} text-ink`}>
        {featured && <Quote className="mb-3 h-5 w-5 text-brand-primary" aria-hidden="true" />}
        {quote}
      </div>
    </article>
  );
    ...
    <>
      <section className="relative overflow-hidden border-y border-surface-border bg-white py-20 sm:py-24">
        <div className="mx-auto max-w-6xl px-6 sm:px-8">
          ...
          <div className="mt-10 grid items-start gap-5 lg:grid-cols-12 lg:gap-6">
            <Reveal variant="left" className="lg:col-span-5">
              <TestimonialCard {...TESTIMONIALS[0]} />
            </Reveal>

            <div className="grid items-start gap-5 sm:grid-cols-2 lg:col-span-7 lg:gap-6">
              {TESTIMONIALS.slice(1).map((item, index) => (
                <Reveal key={item.quote} variant="up" delay={120 + index * 80} className="self-start">
