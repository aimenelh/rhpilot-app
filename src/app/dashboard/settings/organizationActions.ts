...
async function resolveCollectiveAgreement(formData: FormData) {
  ...
  if (requestedIdcc) {
    ...
    if (known) {
      const created = await prisma.collectiveAgreement.create({ data: { id: `ccn-${known.idcc}`, idcc: known.idcc, name: known.name, sourceName: "Code du travail numérique — Ministère du Travail", sourceUrl: "https://code.travail.gouv.fr/outils/convention-collective/entreprise", status: "ACTIVE" } });
    ...
    const created = await prisma.collectiveAgreement.create({ data: { id, idcc: requestedIdcc, name: nameRaw, sourceName: "Code du travail numérique — Ministère du Travail", sourceUrl: "https://code.travail.gouv.fr/outils/convention-collective/entreprise", status: "ACTIVE" } });
    return { id: created.id, name: created.name };
  }
