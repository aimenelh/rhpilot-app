...
export default async function AbsencesPage() {
  ...
  const calendarAbsences = absences.map((absence) => ({
    id: absence.id,
    type: absence.type,
    ...
    <div className="max-w-6xl">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-soft">
          Enregistrez les absences, visualisez le mois et suivez les justificatifs avant transmission à la paie.
        </p>
      </div>
        ...
        <section className="rounded-xl border border-surface-border bg-white p-5">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-xs text-ink-faint">
                Le calendrier donne une vue immédiate des périodes et des motifs. Le détail ci-dessous permet de traiter chaque dossier.
              </p>
            </div>
            <span className="shrink-0 rounded-full bg-surface-subtle px-3 py-1.5 text-xs font-semibold text-ink-soft">
              {absences.length} entrée{absences.length > 1 ? "s" : ""}
            </span>
          </div>

          <AbsenceCalendar absences={calendarAbsences} />

          <div className="mt-6 border-t border-surface-border pt-5">
            <div>
              <h3 className="font-semibold text-ink">Détail et validation</h3>
              <p className="mt-1 text-xs text-ink-faint">
