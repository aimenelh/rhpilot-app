...
import { movePayrollPeriodToReviewAction, type PayrollReviewFormState } from "./periodActions";
...
export default function PayrollReviewButton({
  periodId,
  disabled,
}: {
  periodId: string;
  ...
  const [state, formAction] = useFormState<PayrollReviewFormState, FormData>(
  ...
  return (
    <div className="mt-5 rounded-lg border border-surface-border bg-surface-subtle/30 p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-semibold text-ink">Contrôle de la période</p>
          <p className="mt-1 text-xs text-ink-faint">
            Cette étape ouvre la revue des résultats avant leur validation finale.
          </p>
        </div>
