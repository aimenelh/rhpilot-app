...
export type PayrollPaymentDateFormState = { error: string } | undefined;

export async function setPayrollPaymentDateAction(
  _prevState: PayrollPaymentDateFormState,
  formData: FormData,
): Promise<PayrollPaymentDateFormState> {
  const membership = await getCurrentMembership();
  const user = await getCurrentUser();
  if (!membership || !user) return { error: "Session expirée, veuillez recharger la page." };
  if (!["OWNER", "ADMIN"].includes(membership.accessRole)) return { error: "Seuls les administrateurs peuvent renseigner la date de paiement." };

  const periodId = String(formData.get("periodId") ?? "").trim();
  const rawDate = String(formData.get("paymentDate") ?? "").trim();
  if (!periodId) return { error: "La période de paie est obligatoire." };
  if (!/^\d{4}-\d{2}-\d{2}$/.test(rawDate)) return { error: "La date de paiement est obligatoire et doit être valide." };
  ...
  if (period.status !== "LOCKED") return { error: "La date de paiement ne peut être renseignée qu'après verrouillage de la période." };
