...
export default function PaidLeaveCalculator({ periodId, readOnly }: { periodId: string; readOnly: boolean }) {
    ...
    <section className="border-t border-surface-border bg-white px-5 py-5">
      <div className="flex flex-col gap-1">
        ...
        <p className="max-w-3xl text-xs leading-5 text-ink-faint">
          RH Pilot compare la règle du 1/10e et le maintien de salaire et retient la méthode la plus favorable. Vérifiez la rémunération de référence et les paramètres avant d&apos;ajouter l&apos;indemnité à la paie.
        </p>
      </div>

      <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-5">
        <label className="text-xs font-medium text-ink-soft md:col-span-2 lg:col-span-1">
          Salarié
          <input value={employeeId} onChange={(event) => setEmployeeId(event.target.value)} placeholder="ID salarié" className="mt-1 w-full rounded-lg border border-surface-border bg-white px-3 py-2 text-sm text-ink" />
        </label>
        ...
        <label className="text-xs font-medium text-ink-soft">
          Brut mensuel actuel
          <input value={currentMonthlyGross} onChange={(event) => setCurrentMonthlyGross(event.target.value)} inputMode="decimal" className="mt-1 w-full rounded-lg border border-surface-border bg-white px-3 py-2 text-sm text-ink" />
        </label>
        <label className="text-xs font-medium text-ink-soft">
          Jours de congé
          <input value={daysTaken} onChange={(event) => setDaysTaken(event.target.value)} inputMode="decimal" className="mt-1 w-full rounded-lg border border-surface-border bg-white px-3 py-2 text-sm text-ink" />
        </label>
        <label className="text-xs font-medium text-ink-soft">
          Décompte
          <select value={unit} onChange={(event) => setUnit(event.target.value as "ouvrables" | "ouvres")} className="mt-1 w-full rounded-lg border border-surface-border bg-white px-3 py-2 text-sm text-ink">
            <option value="ouvrables">Jours ouvrables · 30</option>
            <option value="ouvres">Jours ouvrés · 25</option>
          </select>
        </label>
      </div>
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-3">
        <label className="text-xs font-medium text-ink-soft">
          Jours ouvrés du mois pour le maintien
          <input value={workDaysInMonth} onChange={(event) => setWorkDaysInMonth(event.target.value)} inputMode="decimal" className="mt-1 w-full rounded-lg border border-surface-border bg-white px-3 py-2 text-sm text-ink" />
        </label>
        <div className="rounded-lg border border-surface-border bg-surface-subtle/30 px-4 py-3">
          ...
          <p className="mt-1 font-semibold text-ink">{EUR.format(tenthMethod)}</p>
        </div>
        <div className="rounded-lg border border-surface-border bg-surface-subtle/30 px-4 py-3">
          <p className="text-xs text-ink-faint">Maintien de salaire</p>
          <p className="mt-1 font-semibold text-ink">{EUR.format(maintenanceMethod)}</p>
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-col gap-3 rounded-lg border border-accent-teal/20 bg-accent-teal/5 p-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-accent-teal">Montant retenu</p>
          <p className="mt-1 text-lg font-semibold text-ink">{EUR.format(favorable)}</p>
          <p className="mt-1 text-xs text-ink-soft">Méthode la plus favorable : {selectedMethod} · {leaveDays.toFixed(2)} jour{leaveDays > 1 ? "s" : ""}.</p>
        </div>
