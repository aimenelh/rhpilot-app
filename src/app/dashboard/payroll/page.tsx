...
async function createPayrollPeriod(formData: FormData) {
  ...
  if (!["OWNER", "ADMIN"].includes(membership.accessRole)) {
    throw new Error("Seuls les administrateurs peuvent créer une période de paie.");
  }
  ...
  if (!Number.isInteger(year) || year < 2000 || year > 2100) {
    throw new Error("Année de paie invalide.");
  }
  if (!Number.isInteger(month) || month < 1 || month > 12) {
    throw new Error("Mois de paie invalide.");
  }
...
async function savePayrollProfile(formData: FormData) {
  ...
  if (!membership) throw new Error("Organisation introuvable.");
  if (!["OWNER", "ADMIN"].includes(membership.accessRole)) {
    throw new Error("Seuls les administrateurs peuvent configurer la paie.");
  }

  const employeeId = String(formData.get("employeeId") || "");
  const salaryEuros = Number(formData.get("salaryEuros"));
  const monthlyHoursRaw = String(formData.get("monthlyHours") || "151.67");
  const monthlyHours = Number(monthlyHoursRaw);

  if (!employeeId || !Number.isFinite(salaryEuros) || salaryEuros < 0) {
    throw new Error("Salaire mensuel invalide.");
  }
  if (!Number.isFinite(monthlyHours) || monthlyHours <= 0) {
    throw new Error("Nombre d'heures mensuelles invalide.");
  }
  ...
  await prisma.$executeRaw`
    INSERT INTO "payroll_profiles"
      ("id", "organization_id", "employee_id", "base_salary_cents", "monthly_hours", "effective_from")
    VALUES
      (${crypto.randomUUID()}, ${membership.organizationId}, ${employee.id}, ${Math.round(salaryEuros * 100)}, ${monthlyHours}, ${effectiveFrom})
...
export default async function PayrollPage() {
  ...
  const [periods, employees, profileRows] = await Promise.all([
    ...
    prisma.$queryRaw<Array<{ employee_id: string; base_salary_cents: number | null; monthly_hours: number | null }>>`
      SELECT DISTINCT ON ("employee_id") "employee_id", "base_salary_cents", "monthly_hours"
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-ink-soft">
            Le socle paie est en place. On commence par les profils salariés et les périodes, avant de brancher les règles sociales officielles et les calculs complets.
          </p>
        </div>
        <form action={createPayrollPeriod} className="flex items-center gap-2 rounded-xl border border-surface-border bg-white p-2 shadow-sm">
          <select name="month" defaultValue={String(now.getMonth() + 1)} className="rounded-lg border border-surface-border bg-white px-3 py-2 text-sm text-ink">
          ...
          <button type="submit" className="rounded-lg bg-brand-primary px-3 py-2 text-sm font-semibold text-white transition-opacity hover:opacity-90">
            Ouvrir la période
          </button>
        </form>
      </div>
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-surface-border bg-white p-5">
          <p className="text-xs text-ink-faint">Profils paie configurés</p>
          <p className="mt-2 text-2xl font-semibold text-ink">{configuredCount}/{employees.length}</p>
        </div>
        <div className="rounded-xl border border-surface-border bg-white p-5">
          <p className="text-xs text-ink-faint">Période en cours</p>
          <p className="mt-2 text-2xl font-semibold text-ink">{currentPeriod ? currentPeriod.status : "À ouvrir"}</p>
        </div>
      </div>

      <section className="mt-7 rounded-xl border border-surface-border bg-white">
        <div className="border-b border-surface-border px-5 py-4">
          <h2 className="font-semibold text-ink">Profils de paie</h2>
          <p className="mt-1 text-xs text-ink-faint">Le salaire est historisé par date d'effet. Une modification ne réécrit pas les périodes déjà verrouillées.</p>
        </div>
        <div className="divide-y divide-surface-border">
          ...
          {employees.map((employee) => {
            ...
            return (
              <div key={employee.id} className="flex flex-col gap-4 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
                <div className="min-w-0">
                  <p className="font-medium text-ink">{employee.firstName} {employee.lastName}</p>
                  <p className="text-xs text-ink-faint">{employee.position || "Poste non renseigné"}</p>
                </div>
                <div className="flex flex-wrap items-center gap-4">
                  <div className="text-sm text-ink-soft">
                    <span className="text-xs text-ink-faint">Salaire : </span>{formatEuros(profile?.base_salary_cents ?? null)}
                  </div>
                  <div className="text-sm text-ink-soft">
                    <span className="text-xs text-ink-faint">Heures : </span>{profile?.monthly_hours ?? "Non renseigné"}
                  </div>
                  <form action={savePayrollProfile} className="flex items-center gap-2">
                    ...
                    <button type="submit" className="rounded-lg border border-surface-border px-3 py-2 text-sm font-medium text-ink hover:bg-surface-subtle">
                      Enregistrer
                    </button>
                  </form>
                </div>
      ...
      <section className="mt-7 rounded-xl border border-surface-border bg-white">
        <div className="border-b border-surface-border px-5 py-4">
          <h2 className="font-semibold text-ink">Périodes de paie</h2>
          <p className="mt-1 text-xs text-ink-faint">Cycle prévu : brouillon → calculée → contrôle → validée → verrouillée.</p>
        </div>
        <div className="divide-y divide-surface-border">
