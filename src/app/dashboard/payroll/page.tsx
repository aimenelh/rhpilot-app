          ...
          {employees.map((employee) => {
            ...
            return <div key={employee.id} className="flex flex-col gap-4 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-4">
                <div className="text-sm text-ink-soft"><span className="text-xs text-ink-faint">Salaire : </span>{formatEuros(profile?.baseSalaryCents ?? null)}</div>
                <div className="text-sm text-ink-soft"><span className="text-xs text-ink-faint">Heures : </span>{profile?.monthlyHours?.toString() ?? "Non renseigné"}</div>
