...
export function PayrollProfileSection({
  ...
  return (
    <>
      <section className="mt-8 rounded-xl border border-surface-border bg-white">
        <div className="border-b border-surface-border px-5 py-4">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-ink-faint">Profil de rémunération historisé. Les futures évolutions créent une nouvelle date d&apos;effet au lieu d&apos;écraser l&apos;historique.</p>
            </div>
        ...
        {!canEdit ? (
          <div className="grid gap-4 px-5 py-5 sm:grid-cols-2 lg:grid-cols-4">
            <div><p className="text-xs text-ink-faint">Brut mensuel</p><p className="mt-1 text-sm font-semibold text-ink">{formatSalary(profile?.baseSalaryCents ?? null)}</p></div>
          ...
          <form action={formAction} className="space-y-5 px-5 py-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="block"><span className="text-xs font-medium text-ink-soft">Salaire brut mensuel</span><input name="salaryEuros" type="number" min="0" step="0.01" required defaultValue={profile?.baseSalaryCents != null ? (profile.baseSalaryCents / 100).toFixed(2) : ""} className="mt-1.5 w-full rounded-lg border border-surface-border px-3 py-2 text-sm text-ink" /></label>
            ...
            <div className="flex flex-col gap-3 rounded-lg border border-brand-primary/15 bg-brand-primary/5 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-xs leading-5 text-ink-soft">Ces informations serviront de base aux contrôles de paie. Les règles légales et conventionnelles restent calculées depuis les référentiels versionnés.</p>
      ...
      <AlternanceProfileSection employeeId={employeeId} canEdit={canEdit} />
    </>
  );
}
