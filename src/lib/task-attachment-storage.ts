...
export type StoredTaskAttachment = {
  ...
  sha256: string;
  sizeBytes: number;
  mimeType: (typeof ALLOWED_TASK_ATTACHMENT_TYPES)[number];
};

export function storeTaskAttachment(bytes: Buffer, mimeType: string): StoredTaskAttachment {
  if (
    !ALLOWED_TASK_ATTACHMENT_TYPES.includes(
      mimeType as (typeof ALLOWED_TASK_ATTACHMENT_TYPES)[number],
    )
  ) {
  ...
  if (bytes.length > MAX_TASK_ATTACHMENT_BYTES) {
    throw new Error("Le fichier dépasse 4 Mo.");
  }
