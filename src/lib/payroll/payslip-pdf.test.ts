...
describe("payslip PDF", () => {
  ...
  it("passe sur plusieurs pages si beaucoup de variables et de cotisations sont présentes", () => {
    ...
    const pdf = generatePayslipPdf({ ...baseInput, contributions, salary: { ...baseInput.salary, variables } });
    const pdfText = pdf.toString("latin1");
    expect((pdfText.match(/\/Type \/Page/g) ?? []).length).toBeGreaterThan(1);
  });
});
