...
describe("snapshot du contrôle du salaire minimum", () => {
  ...
  it("marque explicitement un salaire non conforme", () => expect(buildMinimumSalaryControlSnapshot({ smic, collectiveMinimum: collective, monthlyHours: 151.67, collectiveRuleVersionId: "ccn-2025-v1", monthlyGrossCents: 210000 })).toMatchObject({ status: "APPLICABLE", source: "COLLECTIVE_AGREEMENT", appliedMonthlyMinimumCents: 213500, compliant: false, differenceCents: -3500 }));

  it("ne masque pas une résolution impossible", () => expect(buildMinimumSalaryControlSnapshot({ smic, collectiveMinimum: { status: "UNRESOLVED", code: "CLASSIFICATION_MISMATCH", message: "Classification absente." }, monthlyHours: 151.67, monthlyGrossCents: 190000 })).toEqual({ status: "UNRESOLVED", explanation: "Le minimum conventionnel n'est pas déterminable : Classification absente.", code: "COLLECTIVE_MINIMUM_UNRESOLVED" }));
