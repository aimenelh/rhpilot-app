...
export function resolveApprenticeshipMinimum(input: ApprenticeshipMinimumInput): AlternanceMinimumResult {
  if (!Number.isInteger(input.age) || input.age < 15 || input.age > 100) return { status: "UNRESOLVED", code: "INVALID_AGE", source: "APPRENTISSAGE_LEGAL", explanation: "L'âge de l'apprenti est nécessaire pour déterminer le minimum légal." };
...
export function resolveProfessionalisationMinimum(input: ProfessionalisationMinimumInput): AlternanceMinimumResult {
  if (!Number.isInteger(input.age) || input.age < 16 || input.age > 100) return { status: "UNRESOLVED", code: "INVALID_AGE", source: "PROFESSIONNALISATION_LEGAL", explanation: "L'âge du salarié est nécessaire pour déterminer le minimum légal." };
