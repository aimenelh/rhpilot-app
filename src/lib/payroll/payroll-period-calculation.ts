...
function calculateWithholdingTax(netBeforeTax: number, withholdingTaxRate: number): number {
  if (!Number.isFinite(withholdingTaxRate) || withholdingTaxRate < 0 || withholdingTaxRate > 1) throw new Error("Le taux de prélèvement à la source est absent ou invalide.");
