import type { AbsencePayrollBasis, AbsencePayrollEffect, AbsencePayrollTreatmentRule } from "./absence-payroll-treatment";
 ...
 * explicitement une base compatible avec le calcul générique actuel.
 * Aucun mode de calcul n'est déduit du type d'absence seul.
 */
export function calculateAbsenceGrossImpact(input: {
  ...
  if (input.rule.basis !== "CALENDAR_DAYS") {
    return { status: "UNSUPPORTED_BASIS" };
  }
