...
export type DsnP26MonthlyInput = {
  ...
  emitter: {
    ...
    contactEmail: string;
    contactPhone: string;
    declaredContactType: string;
    enterpriseApenCode: string;
  };
  establishment: {
    nafCode: string;
  ...
  employees: Array<{
    ...
    contract: {
      ...
      baseSchemeSupplementCode: string;
      collectiveAgreementCode: string;
      sicknessRegimeCode: string;
      workLocationId: string;
      oldAgeRegimeCode: string;
      foreignWorkerCode: string;
      employmentStatusCode: string;
      multipleJobsCode: string;
      multipleEmployersCode: string;
...
function dsnDate(date: Date): string {
  if (!(date instanceof Date) || Number.isNaN(date.getTime())) throw new Error("DSN bloquée : une date nécessaire au fichier est invalide.");
  return `${pad2(date.getUTCDate())}${pad2(date.getUTCMonth() + 1)}${date.getUTCFullYear()}`;
}

function monthBounds(year: number, month: number): { start: Date; end: Date } {
  if (!Number.isInteger(year) || year < 2000 || year > 2100 || !Number.isInteger(month) || month < 1 || month > 12) throw new Error("DSN bloquée : le mois principal déclaré est invalide.");
...
function assertLatin1(value: string, label: string): void {
  for (const character of value) if (character.charCodeAt(0) > 255) throw new Error(`DSN bloquée : ${label} contient un caractère hors ISO-8859-1 (${JSON.stringify(character)}).`);
}
...
function normalizeText(value: string, label: string, optional = false): string | null {
  ...
  if (!normalized) { if (optional) return null; throw new Error(`DSN bloquée : ${label} est absent.`); }
...
function assertDigits(value: string, length: number, label: string): string {
  ...
  if (!new RegExp(`^\\d{${length}}$`).test(normalized)) throw new Error(`DSN bloquée : ${label} doit contenir exactement ${length} chiffres.`);
  return normalized;
}

function assertCode(value: string, label: string, min = 1, max = 20): string {
  const normalized = value.trim().toUpperCase();
  if (normalized.length < min || normalized.length > max || !/^[A-Z0-9.-]+$/.test(normalized)) throw new Error(`DSN bloquée : ${label} est invalide.`);
  return normalized;
}

function assertNaf(value: string, label: string): string {
  const normalized = value.replace(/[.\s]+/g, "").toUpperCase();
  if (!/^\d{4}[A-Z]$/.test(normalized)) throw new Error(`DSN bloquée : ${label} doit respecter le format NAF sur 5 caractères.`);
  return normalized;
}

function assertCountry(value: string, label: string): string {
  const normalized = value.trim().toUpperCase();
  if (!/^[A-Z]{2}$/.test(normalized)) throw new Error(`DSN bloquée : ${label} doit être un code pays ISO sur deux lettres.`);
  return normalized;
}

function add(lines: DsnLine[], code: string, value: string | number | null | undefined): void {
  if (value === null || value === undefined || value === "") return;
  if (!/^[A-Z0-9]+(?:\.[A-Z0-9]+)+$/.test(code)) throw new Error(`DSN bloquée : code de rubrique invalide (${code}).`);
  const rendered = String(value);
  assertLatin1(rendered, `la rubrique ${code}`);
  if (/[\r\n\0]/.test(rendered)) throw new Error(`DSN bloquée : la rubrique ${code} contient un caractère de contrôle.`);
...
function serialize(lines: DsnLine[]): string {
  const rendered = lines.map((line) => {
    ...
    if (Buffer.byteLength(value, "latin1") > 256) throw new Error(`DSN bloquée : la rubrique ${line.code} dépasse la longueur physique maximale de 256 caractères.`);
    return value;
  });
...
export function buildDsnP26V01Monthly(input: DsnP26MonthlyInput): string {
  ...
  const apetCode = assertNaf(input.establishment.nafCode, "le code APET de l'établissement");
  const apenCode = assertNaf(input.emitter.enterpriseApenCode, "le code APEN de l'entreprise");
  const establishmentIdcc = assertDigits(input.establishment.collectiveAgreementCode, 4, "le code IDCC principal de l'établissement");

  if (!Number.isInteger(input.declarationOrder) || input.declarationOrder < 1 || input.declarationOrder > 99) throw new Error("DSN bloquée : le numéro d'ordre de la déclaration doit être compris entre 1 et 99.");
  ...
  for (const employee of input.employees) {
    ...
    const workUnitCode = assertCode(employee.contract.workUnitCode, "l'unité de mesure de la quotité", 2, 2);
    ...
    add(lines, "S21.G00.40.003", assertCode(employee.contract.retirementStatusCode, "le statut retraite complémentaire", 2, 2));
    add(lines, "S21.G00.40.004", assertCode(employee.contract.pcsEsecCode, "le code PCS-ESE", 4, 4));
    add(lines, "S21.G00.40.006", text(employee.position, "l'emploi du salarié"));
    add(lines, "S21.G00.40.007", assertCode(employee.contract.contractNatureCode, "la nature du contrat", 2, 2));
    add(lines, "S21.G00.40.008", assertCode(employee.contract.publicPolicyCode, "le dispositif de politique publique", 2, 2));
    ...
    add(lines, "S21.G00.40.011", workUnitCode);
    ...
    add(lines, "S21.G00.40.014", assertCode(employee.contract.workModalityCode, "la modalité d'exercice du temps de travail", 2, 2));
    add(lines, "S21.G00.40.016", assertCode(employee.contract.baseSchemeSupplementCode, "le complément de base au régime obligatoire", 2, 2));
    add(lines, "S21.G00.40.017", assertDigits(employee.contract.collectiveAgreementCode, 4, "le code de convention collective"));
