...
function parseParameters(parameters: unknown, ruleVersionId: string): {
  ...
  for (const candidate of value.rules) {
    ...
    const candidateRuleVersionId = typeof candidate.ruleVersionId === "string" && candidate.ruleVersionId.trim()
      ? candidate.ruleVersionId.trim()
      : ruleVersionId;

    if (!code || !label || !side || !base || rate === null || !Number.isFinite(rate) || rate < 0 || rate > 1) return null;
  ...
  if (typeof withholdingTaxRate !== "number" || !Number.isFinite(withholdingTaxRate) || withholdingTaxRate < 0 || withholdingTaxRate > 1) return null;
  ...
  if (Array.isArray(value.absenceTreatments)) {
    for (const candidate of value.absenceTreatments) {
      ...
      const effect = candidate.effect === "ADD_TO_GROSS" || candidate.effect === "SUBTRACT_FROM_GROSS" || candidate.effect === "EXCLUDE_FROM_GROSS"
        ? candidate.effect
        : null;
      const basis = candidate.basis === "NONE" || candidate.basis === "CALENDAR_DAYS" || candidate.basis === "WORKING_DAYS" || candidate.basis === "WORKED_HOURS" || candidate.basis === "RULE_DEFINED"
        ? candidate.basis
        : null;
      const candidateRuleVersionId = typeof candidate.ruleVersionId === "string" && candidate.ruleVersionId.trim()
        ? candidate.ruleVersionId.trim()
...
export async function resolvePayrollRuleSetFromPrisma(input: {
  ...
  if (!ruleVersion) {
    return { status: "UNRESOLVED", code: "NO_VALIDATED_RULE_VERSION", message: `Aucune version validée de la règle ${input.code} n'est applicable à la date de paie.` };
  }

  const parsed = parseParameters(ruleVersion.parameters, ruleVersion.id);
  if (!parsed) {
    return { status: "UNRESOLVED", code: "INVALID_RULE_VERSION", message: `La version ${ruleVersion.version} de la règle ${input.code} possède des paramètres invalides.` };
  }
