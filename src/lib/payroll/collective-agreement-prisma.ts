...
async function assertAlternanceMinimum(input: {
  ...
  if (input.collectiveResolution.status === "RESOLVED") {
    const collective = evaluateCollectiveMinimumSalary({
      monthlyGrossCents: input.profile.baseSalaryCents,
      classificationCode: input.profile.classificationCode,
      professionalCategory: input.employee.professionalCategory,
      contractType: input.employee.contractType,
      parameters: input.collectiveResolution.rule.parameters,
    });
  ...
  if (input.profile.baseSalaryCents < (result.monthlyMinimumCents ?? Number.POSITIVE_INFINITY)) {
    throw new Error(`Calcul bloqué pour le salarié ${input.employeeId} : salaire brut mensuel ${input.profile.baseSalaryCents / 100} € inférieur au minimum alternance applicable ${((result.monthlyMinimumCents ?? 0) / 100).toFixed(2)} €. ${result.explanation}`);
  }
}

/**
 * Résolution conventionnelle depuis les données persistées.
 * Cette couche sélectionne uniquement la convention, sa version et sa règle déjà validées.
 */
export async function resolveCollectiveAgreementFromPrisma(input: {
  ...
  const [organization, profile, employee] = await Promise.all([
    ...
    prisma.payrollProfile.findFirst({
      ...
      select: { collectiveAgreementId: true, baseSalaryCents: true, monthlyHours: true, classificationCode: true },
    }),
  ...
  const resolution = resolveCollectiveAgreement({
    ...
    versions: versions.map((version) => ({ ...version, status: normalizeAgreementStatus(version.status) })),
